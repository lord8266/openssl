...
static const char* key_file = "ec_key.pem";
static const char* crt_file = "ec.crt";
static char* cipher_list;
static FILE* keylogfile;
bool isServer;
int tls_version = 3;
static void keylog_cb(const SSL *ssl, const char *line) {
	int n;
    printf("!KEYLOG %d %d", 1,2 );

    
    printf("!KEYLOG! %s\n", line);
    if (isServer){
        if((n=fprintf(keylogfile, "%s\n", line))<0){
            fprintf(stderr, "Unable to write keylog");
        }
        else {
            fflush(keylogfile);
            printf("Written %d bytes to keylog %p\n", n, keylogfile);
        }
    }
}
...
void check_and_load_sess_file(SSL *ssl) {
	char *filepath = "sess1.pem";
	SSL_SESSION *sess;
    BIO *in = BIO_new_file(filepath, "r");
	if (in) {
        sess = PEM_read_bio_SSL_SESSION(in, NULL, 0, NULL);
		if (!sess) {
			fprintf(stderr, "Unable to read session\n");
			return;
		}
		printf("Trying to load old session.. %p\n", sess);
        if (!SSL_set_session(ssl, sess)) {
            fprintf(stderr, "Can't set session\n");
            ERR_print_errors_fp(stderr);
			return;
        }
		printf("Set session %p\n", sess);
	} else {
...
int new_session_cb(SSL *ssl, SSL_SESSION *session) {
	...
	sprintf(buf, "sess%d.pem", i);
	++i;
    BIO *out = BIO_new_file(buf, "w");
	BIO *b_stdout = BIO_new_fp(stdout, BIO_NOCLOSE);
	if (out == NULL) {
		printf("Unable to open file\n");
	}

	printf("received session ssl:%p session:%p\n", ssl, session);

	// Write in pem format
	PEM_write_bio_SSL_SESSION(out, session);
	BIO_free(out);
	// Write to stdout
	// SSL_SESSION_print(b_stdout, session);
	printf("\n");
	return 1;
}
...
SSL_CTX* create_context(bool isServer)
{
	...
	if(!SSL_CTX_set_min_proto_version(ctx, TLS1_3_VERSION)) {
		perror("Unable to set Min Version");
		ERR_print_errors_fp(stderr);
		exit(EXIT_FAILURE);
	}
	if(!SSL_CTX_set_max_proto_version(ctx, TLS1_3_VERSION)) {
		perror("Unable to set Max Version");
		ERR_print_errors_fp(stderr);
		exit(EXIT_FAILURE);
	}
	if (cipher_list) {
    	printf("Setting CIPHERLIST %s\n", cipher_list);
		if (!SSL_CTX_set_ciphersuites(ctx, cipher_list)){
...
void configure_server_context(SSL_CTX *ctx)
{
    ...
    if (SSL_CTX_use_certificate_chain_file(ctx, crt_file) <= 0) {
        ERR_print_errors_fp(stderr);
        exit(EXIT_FAILURE);
    }
...
void configure_client_context(SSL_CTX *ctx)
{
     ...
     */
    /*if (!SSL_CTX_load_verify_locations(ctx, crt_file, NULL)) {
        ERR_print_errors_fp(stderr);
        exit(EXIT_FAILURE);
    }*/
}
...
void run_client_loop() {
    ...
    if (SSL_connect(ssl) == 1) {
		...
		printf("SSL Session was %s, Latest SSL session is %p\n", (SSL_session_reused(ssl)?"reused":"not reused"), session);

        /* Loop to send input from keyboard */
        while (true) {
            /* Get a line of input */
            txlen = getline(&txbuf, &txcap, stdin);
			if (strncmp(txbuf, "keyupdate\n",txlen)==0) {
				printf("Key Update is called\n");
...
int main(int argc, char **argv)
{
	...
	sigaction(SIGPIPE, &(struct sigaction){.sa_handler=SIG_IGN}, NULL); /* ignore SIGPIPE so that server can continue running */

    if(!(keylogfile = fopen("keys.txt", "a"))){
        printf("Unable to open keylogfile!\n");
        exit(EXIT_FAILURE);
    }
    /* Splash */
    ...
    /* Else client */
		...
		run_client_loop(client_skt, ssl_ctx, rem_server_ip);
